...
This project uses the **Depth-Priority Hierarchical AI-INSTRUCT V3** system. The rule files are **not** stored in `.clinerules/` — they are stored under `.ai/` and in per-directory `.ai/instruct.md` files. **The deepest `.ai/instruct.md` always wins.**
